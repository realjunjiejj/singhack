...
def detect(context) -> list[DetectedSignal]:
    ...
    if largest_decline is not None and largest_decline[1] < 0:
        summary += f" {largest_decline[0]} was the largest asset-class decline."
    duration_links = [
        link
        for link in links
        if any("duration" in channel.lower() for channel in link.matched_channels)
        and "yield" in link.description.lower()
    ]
    if duration_links:
        summary += (
            " The event log connects rising yields and duration channels to matching "
            "holdings; this is a transmission path, not measured attribution."
        )
    if links:
        summary += (
...
def _explain_events(context, builder: SignalBuilder, snapshot_items) -> list[EventLink]:
    ...
    for link in material:
        ...
        event_item_ids.append(event_item)
